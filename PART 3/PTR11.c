...
int succ(int i); /* Fungsi suksesor: menambah 1 ke nilai */
int pred(int i); /* Fungsi predesesor: mengurangi 1 dari nilai */
...
/* Fungsi suksesor: menambah 1 ke nilai */
int succ(int i)
{
    return (i + 1);
}

/* Fungsi predesesor: mengurangi 1 dari nilai */
int pred(int i)
{
    return (i - 1);
}
...
void geser(int *TT, int (*f)(int))
{
    ...
    for (i = 0; i < N; i++)
    {
        TT[i] = f(TT[i]); // Memanggil fungsi succ atau pred pada tiap elemen array
        printf("%d ", TT[i]);
    }
    printf("\n");
}